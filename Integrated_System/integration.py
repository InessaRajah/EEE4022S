...
row1 = data_file[data_file.Time == '2016/02/01']
...
data_file = pd.DataFrame(data_file)
data_file = data_file.drop(columns = ['Electricity.Timestep', 'Sum', '[kWh]'])
row2 = data_file[data_file.Time == '2016/02/01']
...
data_file = pd.DataFrame(data_file)
data_file = data_file.drop(columns = ['Electricity.Timestep', 'Sum', '[kWh]'])
row3 = data_file[data_file.Time == '2016/02/01']
...
data_file = pd.DataFrame(data_file)
data_file = data_file.drop(columns = ['Electricity.Timestep', 'Sum', '[kWh]'])
row4 = data_file[data_file.Time == '2016/02/01']
...
data_file = pd.DataFrame(data_file)
data_file = data_file.drop(columns = ['Electricity.Timestep', 'Sum', '[kWh]'])
row5 = data_file[data_file.Time == '2016/02/01']
...
row = data_file[data_file.Date == 20160704]
solar = row['G'].to_list()
...
address = web3.toChecksumAddress('0xDDce2979D319a308851f078B754A0f74c728e85d')
...
# #get battery SOC data from the previous day 
    ...
    data_file = pd.DataFrame(data_file)
    ind = t -1
    SOC = data_file[data_file.Time_Period == ind]
    SOC = SOC['SOC (%)'].to_list()
    num_agents = len(solar_panels)
